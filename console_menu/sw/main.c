...
/// main: Software menu loop - connects to hardware simulator and runs menu
/// Parameters: none (no command-line arguments)
/// Returns: 0 on normal exit, 1 if initialization fails
/// Notes: Creates logical framebuffer, initializes menu, runs update/render loop,
...
int main(void) {
    ...
    while (1) {
        ...
        if (menu_update(&menu, buttons, shm.regs->vsync_counter)) { shm.regs->prop_quit = 1; break; }

        menu_render(&menu, &lfb);
