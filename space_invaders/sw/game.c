...
static void render_alien_explosion(lfb_t *lfb, int cx, int cy, int timer, int points, int clip_to_gameplay) {
	...
	if (timer <= 1) {
		draw_filled_circle_clipped_y(lfb, cx, cy, (base_r > 1) ? (base_r - 1) : 1, BLACK, GAMEPLAY_CLIP_Y_MIN, GAMEPLAY_CLIP_Y_MAX);
	}
}
...
static void render_credits_screen(const game_t *g, lfb_t *lfb) {
	...
	for (int i = 0; i < (int)(sizeof(credits_lines) / sizeof(credits_lines[0])); i++) {
		...
		l_draw_text(lfb, (LW - line_w) / 2, yy + i * CREDITS_LINE_SPACING, credits_lines[i], 1, BLACK);
...
void game_render(game_t *g, lfb_t *lfb) {
	...
	if (g->boss_intro_active) {
		...
		if (g->boss_type == BOSS_TYPE_BLUE) {
			uint32_t blue_flicker_palette[4] = {BLUE, WHITE}; // FIXME TODO
			...
			uint32_t yellow_flicker_palette[4] = {YELLOW, WHITE};
			int flicker_idx = ((BOSS_INTRO_FRAMES - g->boss_intro_timer) / 8) & 3;
			type_color = yellow_flicker_palette[flicker_idx];
		} else if (g->boss_type == BOSS_TYPE_TOWER) {
			uint32_t tower_flicker_palette[4] = {BROWN, WHITE};
			int flicker_idx = ((BOSS_INTRO_FRAMES - g->boss_intro_timer) / 8) & 3;
			type_color = tower_flicker_palette[flicker_idx];
		} else if (g->boss_type == BOSS_TYPE_HERMIT) {
			uint32_t hermit_flicker_palette[4] = {PURPLE, WHITE};
			int flicker_idx = ((BOSS_INTRO_FRAMES - g->boss_intro_timer) / 8) & 3;
			type_color = hermit_flicker_palette[flicker_idx];
		} else if (g->boss_type == BOSS_TYPE_CHARIOT) {
			uint32_t chariot_flicker_palette[4] = {ORANGE, WHITE};
			int flicker_idx = ((BOSS_INTRO_FRAMES - g->boss_intro_timer) / 8) & 3;
	...
	if (g->boss_alive && !g->boss_dying && g->boss_type != BOSS_TYPE_YELLOW) {
		...
		if (!hide_for_chariot_explosion) {
			if (g->boss_prev_x != g->boss_x || g->boss_prev_y != g->boss_y) { 
				draw_sprite1r(lfb, BT, g->boss_prev_x, g->boss_prev_y, BLACK);
			}
	...
	for (int r = 0; r < AROWS; r++) {
		for (int c = 0; c < ACOLS; c++) {
			...
			if (ax != axx || ay != ayy) {
				draw_sprite1r(lfb, AT, axx, ayy, BLACK);
			}
	...
	if (g->ashot.prev_x != g->ashot.x || g->ashot.prev_y != g->ashot.y) {
		if (g->ashot.alive) for (int i = 0; i < 5; i++) l_putpix(lfb, g->ashot.prev_x, g->ashot.prev_y + i, BLACK);
	...
	if (g->boss_shot.prev_x != g->boss_shot.x || g->boss_shot.prev_y != g->boss_shot.y) {
		if (g->boss_shot.alive) for (int i = 0; i < 5; i++) l_putpix(lfb, g->boss_shot.prev_x, g->boss_shot.prev_y + i, BLACK);
